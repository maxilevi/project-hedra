...
namespace Hedra.WorldObjects
{
    ...
    public class Projectile : IDisposable, IWorldObject
    {
        ...
        public Projectile(IEntity Parent, Vector3 Origin, VertexData MeshData)
        {
            ...
            using (var bodyInfo =
                new RigidBodyConstructionInfo(1, new DefaultMotionState(), BulletPhysics.ShapeFrom(dimensions)))
            {
        ...
        public virtual void Update()
        {
            ...
            {
                if (UsePhysics)
                    _body.ApplyCentralImpulse(Propulsion.Compatible() * 50f + Vector3.UnitY.Compatible() *
                        (12.5f + (Utils.Rng.NextFloat() * 8f - 4f) * Falloff));
            ...
            if (Delta.LengthSquared() > 0.05f) Mesh.LocalRotation = Physics.DirectionToEuler(Delta.NormalizedFast());
        ...
        private void HandleMovement()
        {
            ...
            if (HandleLifecycle)
            {
                if (Collide) ProcessWaterCollision();

                if (Lifetime < 0 && !ManuallyDispose) Dispose();
            }
        ...
        {
            if (CollideWithWater && Mesh.Position.Y <= Physics.WaterHeight(Mesh.Position)) InvokeLand(LandType.Water);
        }
        ...
        private void OnCollision(CollisionObject Object0, CollisionObject Object1)
        {
            ...
            if ((other.CollisionFlags & CollisionFlags.NoContactResponse) == CollisionFlags.NoContactResponse &&
                objectInformation.DisableCollisionIfNoContactResponse) return;
            ...
            else if (objectInformation.Entity.Model != null && !objectInformation.Entity.Disposed)
            {
                ...
                if (_parent == entity || _alreadyCollidedList.Contains(entity) ||
                    IgnoreEntities != null && Array.IndexOf(IgnoreEntities, entity) != -1) return;
