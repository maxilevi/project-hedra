...
from Hedra.Items import ItemPool, InventoryExtensions
...
def show_no_bait_msg(human):
    human.MessageDispatcher.ShowNotification(Core.translate("fishing_no_bait"), Colors.ToColorStruct(Colors.Red),
                                             FISHING_ROD_COOLDOWN)
...
def end_pull(human, state):
    ...
    if state['fish']:
        ...
        human.ShowText(human.Position, "+ 1 " + state['fish'].DisplayName.upper(), Colors.ToColorStruct(Colors.Gray),
                       14)
...
def on_rod_active(human, rod, state):
    ...
    if state['on_water']:
        hook.Mesh.LocalPosition = Core.lerp(hook.Mesh.LocalPosition, calculate_hook_offset(state['has_fish']),
                                            Time.DeltaTime * 2.0)
