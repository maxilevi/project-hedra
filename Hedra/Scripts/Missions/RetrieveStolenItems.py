...
def setup_timeline(position, giver, owner, rng):
    ...
    builder.SetReward(create_reward(items))
    return builder
...
def on_found(owner, npc, rng):
    if rng.Next(0, 3) == 1:
        ai = npc.SearchComponent[BasicAIComponent]()
...
def create_items(rng):
    count = rng.Next(1, 4)
    items = []
    options = ItemPool.Matching(lambda x: x.IsWeapon or x.IsArmor)
