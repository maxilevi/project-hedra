...
from System import Array, ArgumentOutOfRangeException, Math
...
def random_items(owner, rng):
    ...
    possible_items = list(filter(lambda x: all(
        [CraftingInventory.GetOutputFromRecipe(r).Name != x.Name or x.Name in owner_recipes for r in recipes]),
                                 possible_items))
...
def recipe_from_item(name, recipes, rng):
    possible_recipes = list(
        filter(lambda r: any([i.Name == name for i in CraftingInventory.GetIngredients(r)]), recipes))
    if possible_recipes:
...
def build_reward(items, rng):
    ...
    def get_random_item():
        n = rng.NextDouble()
        if n < 0.3:
            possibilities = ItemPool.Matching(
                lambda x: x.EquipmentType == EquipmentType.Recipe.ToString() and int(x.Tier) == int(ItemTier.Uncommon))
        elif n < 0.8:
            possibilities = ItemPool.Matching(
                lambda x: x.EquipmentType == EquipmentType.Recipe.ToString() and int(x.Tier) == int(ItemTier.Common))
        else:
