...
namespace Hedra.Engine.StructureSystem.VillageSystem.Builders
{
    ...
    public abstract class Builder<T> : Builder where T : IBuildingParameters
    {
        protected Builder(CollidableStructure Structure)
        {
            ...
            VillageObject = Structure.WorldObject;
        }
        ...
        {
            return PlaceGroundwork(Parameters.Position, ModelRadius(Parameters, Cache) * .75f);
        }
        ...
        public virtual BuildingOutput Build(T Parameters, DesignTemplate Design, VillageCache Cache, Random Rng,
        ...
        {
            return Vector3.Transform((Position + Parameters.Design.Offset) * Parameters.Design.Scale, Transformation) +
                   Parameters.Position;
        }

        private static void AddStructure<U>(T Parameters, U[] Templates, Matrix4x4 Transformation,
            BuildingOutput Output, Func<Vector3, U, BaseStructure> Get) where U : IPositionable
        {
            for (var i = 0; i < Templates.Length; ++i)
                Output.Structures.Add(Get(TransformPosition(Parameters, Templates[i].Position, Transformation),
                    Templates[i]));
        }
        ...
        protected void AddChimneys(T Parameters, ChimneyTemplate[] Chimneys, Matrix4x4 Transformation,
            BuildingOutput Output)
        {
            AddStructure(Parameters, Chimneys, Transformation, Output, (P, T) => new Chimney(P));
        }

        protected void AddGenericStructure(T Parameters, StructureTemplate[] Templates, Matrix4x4 Transformation,
            BuildingOutput Output)
        {
            AddStructure(Parameters, Templates, Transformation, Output,
                (P, T) => StructureTemplate.FromType(T.Type, P));
        }
        ...
        protected void AddDoors(T Parameters, VillageCache Cache, DoorTemplate[] Doors, Matrix4x4 Transformation,
            BuildingOutput Output)
        {
        ...
        public static Door CreateDoor(VertexData Model, Vector3 Position, Vector3 DoorPosition, Vector3 Scale,
            Matrix4x4 Transformation, CollidableStructure Structure, bool InvertedRotation = false,
            bool InvertedPivot = false)
        {
            var vertexData = Model.Scale(Scale);
            var rotationPoint =
                Vector3.Transform(Door.GetRotationPointFromMesh(vertexData, InvertedPivot), Transformation);
        ...
        {
            var rotationMatrix = LookAtCenter
        ...
        protected IHumanoid SpawnVillager(Vector3 Position, Random Rng)
        {
            if (!(VillageObject is Village village))
                throw new ArgumentOutOfRangeException(
                    $"In order to spawn a villager, the structure needs to be a village but is a '{VillageObject}'");
        ...
        {
            return PushGroundwork(CreateGroundwork(Position, Radius, Type));
        }
        ...
        protected bool PushGroundwork(GroundworkItem Item)
        {
            ...
            if (Item.Groundwork != null) Structure.AddGroundwork(Item.Groundwork);
            return true;
        }
