...
namespace Hedra.Engine.StructureSystem.VillageSystem
{
    public class GridPlacementDesigner : PlacementDesigner
    {
        ...
        public GridPlacementDesigner(VillageRoot Root, VillageConfiguration Config, Random Rng) : base(Root, Config,
            Rng)
        {
        ...
        public override PlacementDesign CreateDesign()
        {
            ...
            {
                if ((new Vector2(x, z) - new Vector2(VillageSize, VillageSize) * .5f).LengthSquared() >
                    VillageSize * .5f * .5f * VillageSize)
                    continue;
        ...
        {
            return SpecialPoints.All(S =>
                (Position - S.Position.Xz()).LengthSquared() > Math.Pow(S.GetSize(Root.Cache) * 1.5f, 2));
        }
        ...
        private IBuildingParameters[] AddSpecialPoints(PlacementDesign Design)
        {
            ...
            {
                new Pair<IPlacer<IBuildingParameters>, Action<IBuildingParameters>>(MayorPlacer,
                    P => Design.Generics.Add((GenericParameters)P)),
                new Pair<IPlacer<IBuildingParameters>, Action<IBuildingParameters>>(ShopPlacer,
                    P => Design.Generics.Add((GenericParameters)P)),
                new Pair<IPlacer<IBuildingParameters>, Action<IBuildingParameters>>(ClothierPlacer,
                    P => Design.Generics.Add((GenericParameters)P)),
                new Pair<IPlacer<IBuildingParameters>, Action<IBuildingParameters>>(MasonryPlacer,
                    P => Design.Generics.Add((GenericParameters)P)),
                new Pair<IPlacer<IBuildingParameters>, Action<IBuildingParameters>>(InnPlacer,
                    P => Design.Generics.Add((GenericParameters)P))
            ...
            {
                var randomPosition = new Vector2(VillageDesign.Spacing * Rng.NextFloat() * VillageSize,
                    VillageDesign.Spacing * Rng.NextFloat() * VillageSize) * .25f;
        ...
        private bool SelectPlacer(PlacementPoint Point, PlacementDesign Design)
        {
            ...
            if (distFromCenter > SparseZone)
            {
                ...
                var mod = (1.0f + Math.Max(0, distFromCenter - SparseZone) * 2f) * VillageDesign.Spacing *
                          Point.Position.NormalizedFast();
                Point.Position = (new Vector2(VillageDesign.Spacing * x, VillageDesign.Spacing * z + spacingX) -
                    offset + mod.Xz()).ToVector3();
            }
        ...
        private static void TryAddAny(Pair<IPlacer<IBuildingParameters>, Action<IBuildingParameters>>[] Pairs,
        ...
        public override void FinishPlacements(CollidableStructure Structure, PlacementDesign Design)
        {
            ...
            for (var z = 1; z < VillageSize; ++z)
            {
                ...
                var offset = CalculateOffset();
                var startPosition = new Vector2(VillageDesign.Spacing * x + spacing * .5f,
                    VillageDesign.Spacing * z + spacing * .5f + addX) - offset;
                var distFromCenter =
            ...
            for (var i = 1; i < _specialPoints.Length; ++i)
                Structure.AddGroundwork(new RoundedGroundwork(_specialPoints[i].Position,
                    _specialPoints[i].GetSize(Root.Cache), BlockType.StonePath)
                {
                    BonusHeight = .25f
                });
        ...
        private void PlaceDecorations(CollidableStructure Structure, PlacementDesign Design)
        {
            ...
            for (var i = 0; i < vertices.Length; ++i)
            {
                ...
                if (Rng.Next(0, 5) == 1)
                {
                    ...
                    Vector2 EdgeDirection(GraphEdge E)
                    {
                        return (vertices[E.GetOtherVertex(k)] - vertices[k]).Normalized();
                    }
                    ...
                    DecorationsPlacer.PlaceBench(vertices[i].ToVector3(), inIntersection, orientation, Structure, Root,
                        Rng);
                }
            }
        }
