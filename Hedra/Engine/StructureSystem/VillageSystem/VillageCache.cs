...
namespace Hedra.Engine.StructureSystem.VillageSystem
{
    public class VillageCache
    {
        ...
        public static VillageCache FromTemplate(VillageTemplate Template)
        {
            ...
            for (var j = 0; j < designs[i].Length; j++)
            {
                ...
                cache._colliderCache.Add(
                    designs[i][j].Path,
                    AssetManager.LoadCollisionShapes(designs[i][j].Path, Vector3.One * designs[i][j].Scale)
                        .Select(S => S.Transform(offsetMatrix)).ToList()
                );
                cache._modelCache.Add(
                    designs[i][j].Path,
                    AssetManager.PLYLoader(designs[i][j].Path, Vector3.One * designs[i][j].Scale, offset, Vector3.Zero)
                        .AsCompressed()
                );
                cache._sizeCache.Add(designs[i][j].Path,
                    CalculateBounds(cache._modelCache[designs[i][j].Path].ToVertexData()));
                if (designs[i][j].LodPath != null)
                    cache._modelCache.Add(designs[i][j].LodPath,
                        AssetManager.PLYLoader(designs[i][j].LodPath, Vector3.One * designs[i][j].Scale)
                            .AsCompressed());
            }
        ...
        private static VertexData LoadCollisionShapesAsVertexData(string Filename, Vector3 Scale, Vector3 Position,
            Vector3 Rotation)
        {
