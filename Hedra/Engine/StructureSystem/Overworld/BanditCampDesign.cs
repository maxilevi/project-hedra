...
namespace Hedra.Engine.StructureSystem.Overworld
{
    ...
    {
        public override int StructureChance => StructureGrid.BanditCampChance;
        ...
        private bool IntersectsWithOtherCampfires(List<TentParameters> OccupiedSpots, Vector3 NewSpot)
        {
            for (var i = 0; i < OccupiedSpots.Count; i++)
                if ((OccupiedSpots[i].WorldPosition - NewSpot).LengthSquared() < 16 * 16)
                    return true;
            return false;
        }
        ...
        {
            var structure = base.Setup(TargetPosition, Rng, new BanditCamp(TargetPosition, PlateauRadius));
            var scaleMatrix = Matrix4x4.CreateScale(3 + Rng.NextFloat() * 1.5f);
            var tents = SetupTents(TargetPosition, scaleMatrix, Rng);
        ...
        private TentParameters[] SetupTents(Vector3 TargetPosition, Matrix4x4 ScaleMatrix, Random Rng)
        {
            ...
            {
                var dist = (22 + Rng.NextFloat() * 6f) * Chunk.BlockSize;
            ...
            for (var i = 0; i < randomCampfires; i++)
            {
                ...
                if (IntersectsWithOtherCampfires(tents, newPosition) ||
                    (TargetPosition - newPosition).LengthFast() < 6 * Chunk.BlockSize) continue;
        ...
        {
            return Translations.Get("quest_complete_structure_short_bandit_camp",
                Structure.Rescuee.Name.ToUpperInvariant());
        }

        protected override string GetDescription(BanditCamp Structure)
        {
            return Translations.Get("quest_complete_structure_description_bandit_camp",
                Structure.Rescuee.Name.ToUpperInvariant(), DisplayName, Structure.EnemiesLeft);
        }
