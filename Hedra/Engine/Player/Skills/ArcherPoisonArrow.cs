...
namespace Hedra.Engine.Player
{
	...
	/// </summary>
	public class PoisonArrow : BaseSkill
	{
		private Animation ShootAnimation;
		private float BaseDamage = 35f, Damage;
		
		public PoisonArrow(Vector2 Position, Vector2 Scale, Panel InPanel, LocalPlayer Player) : base(Position, Scale, InPanel, Player) {
			...
			ShootAnimation.OnAnimationMid += delegate {
				...
				if(Player.Model.LeftWeapon is Bow){
					...
					Arrow.MoveEventHandler += delegate { 
						Arrow.Mesh.Tint = Bar.Poison * new Vector4(1,3,1,1);
					};
