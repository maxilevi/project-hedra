...
namespace Hedra.Engine.Player.AbilityTreeSystem
{
    ...
    public class AbilityTreeBlueprint
    {
        ...
        {
            Identifier = AbilityTreeTemplate.Name.ToLowerInvariant();
            ...
            for (var i = 0; i < Items.Length; i++)
            {
                ...
                for (var j = 0; j < Items[i].Length; j++)
                {
                    ...
                    Items[i][j] = skill != null
                        ? new TreeItem
                        {
                            AbilityType = SkillFactory.Instance.Get(skill),
                            Image = GameManager.Player.Toolbar.Skills
                                .First(S => S.GetType() == SkillFactory.Instance.Get(skill)).IconId
                        }
                        : new TreeItem();
                }
            }
        ...
        public string DisplayName => Translations.Get(Identifier);
