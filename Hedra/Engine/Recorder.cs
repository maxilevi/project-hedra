...
namespace Hedra.Engine
{
    ...
    public static class Recorder
    {
        ...
        public static string SaveScreenshot(string Path)
        {
            ...
            int w = (int) GameSettings.SurfaceWidth;
            int h = (int) GameSettings.SurfaceHeight;
            int[] readPixels = new int[w * h];
            Renderer.ReadPixels(0, 0, w, h, PixelFormat.Rgba, PixelType.Byte, readPixels);
            ...
            for (int i = 0; i < readPixels.Length; i++)
            {
                ...
                int g = (p >> 8) & 0xff;
                int b = (p >> 16) & 0xff;
                int val = ((r << 16 | g << 8 | b) << 1);
                readPixels[i] = (byte) ((r << 16 | g << 8 | b) << 1);
            }
            ...
            using (Bitmap Bmp = new Bitmap(w, h))
            {
                ...
                Marshal.Copy(readPixels, 0, data.Scan0, readPixels.Length);
                Bmp.UnlockBits(data);
