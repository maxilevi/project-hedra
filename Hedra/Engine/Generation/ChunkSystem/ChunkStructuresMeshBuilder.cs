    ...
    public class ChunkStructuresMeshBuilder
    {
        ...
        public ChunkMeshBuildOutput AddStructuresMeshes(IAllocator Allocator, ChunkMeshBuildOutput Input, int Lod)
        {
            ...
            for (var i = 0; i < staticElements.Count; i++)
            {
                ...
                AddManually(
                    ...
                    Input.StaticData.Extradata, Input.StaticData.Indices,
            ...
            for (var i = 0; i < instanceElements.Length; i++)
            {
                ...
                ProcessInstanceData(Allocator, instanceElements[i], nativeStatic, i, Lod, distribution,
                    instanceElements[i].CanSimplifyProgramatically ? LODMap[Lod] : -1);
            }
            ...
            AddManually(
                Input.StaticData.Vertices, Input.StaticData.Normals, Input.StaticData.Colors,
                Input.StaticData.Extradata, Input.StaticData.Indices,
                nativeStatic.Vertices, nativeStatic.Normals, nativeStatic.Colors, nativeStatic.Extradata,
                nativeStatic.Indices
            );
            AddManually(
                Input.InstanceData.Vertices, Input.InstanceData.Normals, Input.InstanceData.Colors,
                Input.InstanceData.Extradata, Input.InstanceData.Indices,
                nativeInstance.Vertices, nativeInstance.Normals, nativeInstance.Colors, nativeInstance.Extradata,
                nativeInstance.Indices
            );
        ...
        private static void PackAndAddModel(NativeVertexData InstanceModel, NativeVertexData Model)
        {
            ...
            AddManually(
                ...
                InstanceModel.Vertices, InstanceModel.Normals, InstanceModel.Colors, InstanceModel.Extradata,
                InstanceModel.Indices
            );
        }

        private static void AddManually(IList<Vector3> Vertices, IList<Vector3> Normals, IList<Vector4> Colors,
            IList<float> Extradata, IList<uint> Indices,
            IList<Vector3> NewVertices, IList<Vector3> NewNormals, IList<Vector4> NewColors, IList<float> NewExtradata,
        ...
        private void AssertValidModel(NativeVertexData Model)
        {
            ...
            if (Model.Colors.Count != Model.Extradata.Count)
                throw new ArgumentOutOfRangeException(
                    $"Extradata '{Model.Extradata.Count}' or color '{Model.Colors.Count}' mismatch");
        ...
        private void SetColor(NativeVertexData Model, InstanceData Element, int Index, RandomDistribution Distribution)
        {
            ...
            if (Model.Colors.Count != Model.Vertices.Count)
            {
                ...
                Log.WriteLine(
                    $"Removed instance data '{Model.Name}' because of mismatch. ({Model.Colors.Count}|{Model.Vertices.Count})");
                return;
            ...
            if (Element.GraduateColor) Model.GraduateColor(Vector3.UnitY);
        }
