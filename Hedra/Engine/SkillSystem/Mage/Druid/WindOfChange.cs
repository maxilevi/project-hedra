...
namespace Hedra.Engine.SkillSystem.Mage.Druid
{
    public class WindOfChange : ActivateDurationSkillWithSingleAnimation
    {
        ...
        protected override Animation SkillAnimation { get; } =
            AnimationLoader.LoadAnimation("Assets/Chr/MageStaffGroundHit.dae");
        ...
        private void SpawnEffect()
        {
            ...
            _particles.Position = User.Position + Vector3.UnitY * User.Model.Height * .5f -
                                  User.LookingDirection.Xz().ToVector3() * 6f;
        ...
        {
            SkillUtils.DoNearby(User, PushDistance, 0,
                (E, F) =>
                {
                    E.Physics.MoveTowards(-(User.Position - E.Position).Xz().NormalizedFast().ToVector3() * 80f);
                });
        }
    }
}