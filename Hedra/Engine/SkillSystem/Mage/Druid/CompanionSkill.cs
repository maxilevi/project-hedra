...
namespace Hedra.Engine.SkillSystem.Mage.Druid
{
    public abstract class CompanionSkill : SingleAnimationSkill<IPlayer>
    {
        ...
        protected sealed override Animation SkillAnimation { get; } =
            AnimationLoader.LoadAnimation("Assets/Chr/ArcherWhistle.dae");
        ...
        public override string[] Attributes
        {
            get
            {
                ...
                var masterySkill =
                    (CompanionMastery)User.Toolbar.Skills.First(S => S.GetType() == typeof(CompanionMastery));
                return new[]
                {
                    Translations.Get("companion_mastery_health_change",
                        (template.MaxHealth * masterySkill.HealthMultiplier).ToString("0.0",
                            CultureInfo.InvariantCulture)),
                    Translations.Get("companion_mastery_damage_change",
                        (template.AttackDamage * masterySkill.DamageMultiplier).ToString("0.0",
                            CultureInfo.InvariantCulture))
                };
            }
        }
        ...
        protected virtual IEntity SpawnMinion()
        {
            ...
            minion.AddComponent(new HealthBarComponent(minion, Translations.Get($"{Keyword}_companion_name"),
