...
namespace Hedra.Engine.SkillSystem
{
    ...
    {
        public static void DamageNearby(IEntity Caster, float Damage, float Radius, float Angle = 0,
            bool PlaySound = true)
        {
        ...
        public static void DoNearby(IEntity Caster, Vector3 Position, float Radius, float Angle,
            Action<IEntity, float> Lambda)
        {
        ...
        public static IEntity GetNearest(IEntity Caster, Vector3 Direction, float Angle, float Radius,
            Func<IEntity, bool> Lambda)
        {
            var entities = World.Entities;
            var highest = -1f;
            IEntity entity = null;
            for (var i = 0; i < entities.Count; ++i)
            {
                ...
                if (Lambda(entities[i]) && dot > Angle && dot > highest &&
                    (entities[i].Position - Caster.Position).LengthSquared() < Radius * Radius)
                {
                    entity = entities[i];
                    highest = dot;
                }
        ...
        public static bool IsBehind(IEntity Caster, IEntity Victim)
        {
            return Caster.InAttackRange(Victim, 2f) && Vector3.Dot(Victim.Orientation, Caster.Orientation) > -.75f
                                                    && Vector3.Dot((Victim.Position - Caster.Position).Normalized(),
                                                        Caster.Orientation) > .75f;
        }
