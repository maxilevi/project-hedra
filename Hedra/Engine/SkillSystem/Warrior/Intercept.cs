...
namespace Hedra.Engine.SkillSystem.Warrior
{
    public class Intercept : CappedSkill<IPlayer>
    {
        ...
        private float Damage => BaseDamage * (Level * 0.15f) + BaseDamage;
        public override float MaxCooldown => Math.Max(BaseCooldown - 0.80f * Level, CooldownCap);
        private float Duration => Math.Max(BaseDuration - 0.01f * Level, DurationCap);
        ...
        public override void Update()
        {
            ...
            if (!_isMoving) return;
            if (User.Model.AnimationBlending == null) User.Model.BlendAnimation(_interceptStance);

            if (_dmgTimer.Tick())
                World.Entities.ToList().ForEach(delegate(IEntity Entity)
                {
                    if ((Entity.Position - User.Position).LengthSquared() < 4 * 4 && !Entity.IsKnocked &&
                        User != Entity)
                    {
        ...
        private void EmitParticles()
        {
            ...
            if (underChunk != null)
            {
                ...
                World.Particles.PositionErrorMargin =
                    new Vector3(User.Model.Dimensions.Size.X, 2, User.Model.Dimensions.Size.Z) * .5f;
            }
        ...
        protected override void DoUse()
        {
            ...
            User.Movement.Move(
                User.Physics.MoveFormula(User.View.LookingDirection.Xz().ToVector3().NormalizedFast()) * 1.5f, Duration,
                false);
        }
    }
}