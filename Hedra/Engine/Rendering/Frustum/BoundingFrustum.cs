...
namespace Hedra.Engine.Rendering.Frustum
{
    public class BoundingFrustum
    {
        ...
        {
            var size = (Max - Min) * .5f;
            var r = Math.Abs(size.X * Plane.Normal.X) + Math.Abs(size.Y * Plane.Normal.Y) +
        ...
        public void Draw(Matrix4x4 DrawingMatrix)
        {
            if (GameSettings.DebugFrustum)
            {
                ...
                var newCorners = UnitCube.Select(V => Vector4.Transform(V, invProjection))
                    .Select(P => P.Xyz() / P.W + position).Select(P => Vector3.Transform(P, DrawingMatrix)).ToArray();
                for (var i = 0; i < newCorners.Length; ++i) BasicGeometry.DrawPoint(newCorners[i], Vector4.One, 10);

                for (var i = 0; i < _planes.Length; ++i)
                    BasicGeometry.DrawLine(DrawingMatrix.ExtractTranslation() + _planes[i].Normal * 32,
                        DrawingMatrix.ExtractTranslation() + _planes[i].Normal * 48, Vector4.One, 5);
            }
        }
    }
}