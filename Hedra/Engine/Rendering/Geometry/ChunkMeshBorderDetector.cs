    ...
    public class ChunkMeshBorderDetector
    {
        ...
        public MeshBorderOutput Process(Vector3[] TerrainMesh, Vector3 Start, Vector3 End)
        {
            ...
            for (var i = 0; i < TerrainMesh.Length; i++)
            {
                ...
                if (Math.Abs(vertex.X - Start.X) < Epsilon && Math.Abs(vertex.Z - Start.X) < Epsilon)
                    backLeftCorner.Add(vertex);
                else if (Math.Abs(vertex.X - End.X) < Epsilon && Math.Abs(vertex.Z - Start.Z) < Epsilon)
                    backRightCorner.Add(vertex);
                else if (Math.Abs(vertex.X - Start.X) < Epsilon && Math.Abs(vertex.Z - End.Z) < Epsilon)
                    frontLeftCorner.Add(vertex);
                else if (Math.Abs(vertex.X - End.X) < Epsilon && Math.Abs(vertex.Z - End.Z) < Epsilon)
                    frontRightCorner.Add(vertex);

                else if (Math.Abs(vertex.X - Start.X) < Epsilon && vertex.Z > Start.Z && vertex.Z < End.Z)
                    left.Add(vertex);
                else if (Math.Abs(vertex.X - End.X) < Epsilon && vertex.Z > Start.Z && vertex.Z < End.Z)
                    right.Add(vertex);
                else if (Math.Abs(vertex.Z - Start.Z) < Epsilon && vertex.X > Start.X && vertex.X < End.X)
                    back.Add(vertex);
                else if (Math.Abs(vertex.Z - End.Z) < Epsilon && vertex.X > Start.X && vertex.X < End.X)
                    front.Add(vertex);
            }
        ...
        public uint[] ProcessEntireBorder(NativeVertexData Mesh, Vector3 Start, Vector3 End)
        {
            ...
            for (var i = 0; i < Mesh.Indices.Count; i++)
            {
                ...
                if (Math.Abs(vertex.Z - Start.Z) < epsilon || Math.Abs(vertex.Z - End.Z) < epsilon ||
                    Math.Abs(vertex.X - Start.X) < epsilon || Math.Abs(vertex.X - End.X) < epsilon)
                    all.Add((uint)index);
            }
