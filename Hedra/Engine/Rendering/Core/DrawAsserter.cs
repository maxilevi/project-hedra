...
namespace Hedra.Engine.Rendering.Core
{
    public static class DrawAsserter
    {
        ...
        public static void AssertMultiDrawElement(PrimitiveType Primitive, uint[] Counts, DrawElementsType ElementsType,
            IntPtr[] Offsets, int Length)
        {
            return;
            if (Counts.Length != Offsets.Length)
                throw new ArgumentException(
                    $"Found difference in counts ('{Counts.Length}') and offsets ('{Offsets.Length}') arrays");
        ...
        public static void AssertDrawElements(PrimitiveType Primitive, int Count, DrawElementsType ElementsType,
            IntPtr Offset)
        {
            return;
            BaseAssert();
        ...
        public static void AssertDrawElementsInstanced(PrimitiveType Primitive, int Count,
            DrawElementsType ElementsType, IntPtr Indices, int InstanceCount)
        {
            return;
            BaseAssert();
            AssertElementBuffer();
        ...
        public static void AssertDrawArrays(PrimitiveType Primitive, int Offset, int Count)
        {
            ...
            if (Offset > Count)
                throw new ArgumentException(
                    $"Draw offset '{Offset}' cannot be higher than the amount of elements '{Count}'");
        ...
        private static void AssertDrawType(DrawElementsType Type)
        {
            ...
            if (vbo.ElementType != expectedType)
                throw new ArgumentOutOfRangeException(
                    $"Draw method was called with '{Type}' but the indices are of type '{vbo.ElementType}'");
        }
        ...
        private static void AssertCount(int Count)
        {
            ...
            if (Count >= indicesVBO.Count)
                throw new ArgumentException(
                    $"Found unexpected count ('{Count}') and indices count ('{indicesVBO.Count}')");
        }
        ...
        private static void AssertVBO(PrimitiveType Primitive, VBO Buffer, int Mod)
        {
            if (Buffer.Count % Mod != 0)
                throw new ArgumentOutOfRangeException(
                    $"Drawing with primitive '{Primitive}' requires your vbos to be a multiple of '{Mod}'");
        }
        ...
        private static void AssertInputs(Shader Shader, VAO VAOBound)
        {
            ...
            if (Renderer.VertexAttributeHandler.Count != inputs.Length)
                throw new ArgumentException(
                    $"The amount of enabled inputs '({Renderer.VertexAttributeHandler.Count})' differs from the amount of shader inputs '({inputs.Length})'");

            for (var i = 0; i < inputs.Length; ++i)
                if (!Renderer.VertexAttributeHandler.IsEnabled(inputs[i].Location))
                    throw new ArgumentException(
                        $"Shader '{Shader.Name}' expects input '{inputs[i].Name}' of type '{inputs[i].Type}' at location '{inputs[i].Location}' to be enabled but its disabled.");
            ...
            for (var i = 0; i < Renderer.VertexAttributeHandler.Count; ++i)
                if (inputs[i].Type != types[i])
                    throw new ArgumentException(
                        $"Shader '{Shader.Name}' has input '{inputs[i].Name}' of type '{inputs[i].Type}'' that differs from expected type '{types[i]}'");
        }

        private static void AssertOutputs(Shader ShaderBound, FBO FBOBound)
        {
            var outputs = ShaderBound.Outputs;
            if (FBOBound != null && FBOBound.Attachments.Length > outputs.Length ||
                FBOBound == null && outputs.Length != 1)
                throw new ArgumentException(
                    $"The amount of enabled outputs '({outputs.Length})' from shader '{ShaderBound.Name}' differs from the amount of fbo inputs '({FBOBound?.Attachments.Length ?? 1})'");
            ...
            if (FBOBound != null)
            {
                for (var i = 0; i < FBOBound.Attachments.Length; ++i)
                {
                    ...
                    if (outputs[i].Type != parsedType)
                        throw new ArgumentException(
                            $"FBO input with type '{parsedType}' differs from shader output type '{outputs[i].Type}'");
                }
            }
            else
            {
                if (outputs[0].Type != typeof(Vector4))
                    throw new ArgumentException(
                        $"Shader output type '{outputs[0].Type}' differs from the expected default fbo output '{typeof(Vector4)}'");
            }
        }
    }
}