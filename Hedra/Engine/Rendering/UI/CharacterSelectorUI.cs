...
namespace Hedra.Engine.Rendering.UI
{
    ...
    public class CharacterSelectorUI : Panel, IUpdatable
    {
        ...
        public CharacterSelectorUI()
        {
            ...
            var blackBand2 = new BackgroundTexture(Color.FromArgb(255, 69, 69, 69), Color.FromArgb(255, 19, 19, 19),
                -bandPosition, UserInterface.BlackBandSize, GradientType.LeftRight);

            var currentTab = new GUIText(Translation.Create("choose_character"), new Vector2(0f, bandPosition.Y),
            ...
            newChr.Click += delegate
            {
                Disable();
                GameManager.Player.UI.CharacterCreator.Enable();
            };

            var playBtn = new Button(new Vector2(-.1f, -.8f), Vector2.One, Translation.Create("load"), Color.White,
                FontCache.GetNormal(14));
            playBtn.Click += OnSelect;

            var deleteButton = new Button(new Vector2(.1f, -.8f), Vector2.One, Translation.Create("delete"),
                Color.White, FontCache.GetNormal(14));
            ...
            _dataElements.Add(_name);
            _dataElements.Add(_level);
            _dataElements.Add(playBtn);
            _dataElements.Add(deleteButton);
            for (var i = 0; i < _dataElements.Count; i++) AddElement(_dataElements[i]);
            AddElement(playBtn);
            AddElement(newChr);
            AddElement(blackBand);
            AddElement(blackBand2);
        ...
        {
            for (var i = 0; i < _humans.Count; i++) _humans[i].Dispose();
            _humans.Clear();
        }
            ...
            for (var i = 0; i < _humans.Count; i++)
            {
                ...
                if (_previousHuman != null)
                    for (var k = 0; k < _humans.Count; k++)
                    {
                        ...
                        _previousHuman.Model.TargetRotation = new Vector3(0,
                            Physics.DirectionToEuler(FireDirection(k, 8).NormalizedFast().Xz().ToVector3()).Y + 180, 0);
                    }
                ...
                _level.Text =
                    $"{Translations.Get(_selectedHuman.Class.ToString().ToLowerInvariant())} {Translations.Get("level")} {_selectedHuman.Level}";
                break;
            }
        }
        ...
        private void HandleHovering()
        {
            ...
            for (var i = 0; i < _humans.Count; i++)
            {
                var space = Vector4.Transform(new Vector4(_humans[i].Position + Vector3.UnitY * 6f, 1),
                ...
                if (_humans[i].Model.Enabled && Math.Abs(ndc.X - coords.X) < size.X &&
                    Math.Abs(1 - ndc.Y - coords.Y) < size.Y)
                {
        ...
        private void HandlePreviousSelection()
        {
                ...
                .LengthSquared() > 1 * 1)
            {
                ...
                _previousHuman.Model.LocalRotation =
                    Physics.DirectionToEuler(backTarget.NormalizedFast().Xz().ToVector3());
