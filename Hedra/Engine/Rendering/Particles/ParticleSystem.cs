...
namespace Hedra.Engine.Rendering.Particles
{
    public class ParticleSystem : IRenderable, IDisposable, ITickable
    {
        ...
        public void Draw()
        {
            if (!HasMultipleOutputs && (Position - LocalPlayer.Instance.Position).LengthSquared() >
                GeneralSettings.DrawDistanceSquared) return;
            if (ParticleCount > 0)
            {
            ...
            lock (_lock)
            {
                for (var i = 0; i < _particles.Count; i++)
                {
                    var randomRotation = RandomRotation
                        ...
                        : Vector3.Zero;
                    if (!_particles[i].Update(DeltaTime, randomRotation)) _particles.RemoveAt(i);
                }
        ...
        public void Emit()
        {
            if ((Position - LocalPlayer.Instance.Position).LengthSquared() >
                GeneralSettings.DrawDistanceSquared) return;
            ...
            else if (Shape == ParticleShape.Sphere)
            {
                if (particlePosition.X * particlePosition.X + particlePosition.Y * particlePosition.Y +
                    ...
                    lock (_lock)
                    {
                        _particles.Add(new Particle3D(Position + particlePosition, Direction * 25,
                            Mathf.RandomVector3(Utils.Rng) * 360,
                            newColor,
                            Scale + particleScale, GravityEffect, ParticleLifetime, Collides));
                    }
        ...
        {
            _particleVbo = new VBO<Vector4>(new Vector4[0], 0, VertexAttribPointerType.Float, BufferTarget.ArrayBuffer,
