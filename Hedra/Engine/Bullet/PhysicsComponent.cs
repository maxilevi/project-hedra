...
namespace Hedra.Engine.Bullet
{
    ...
    public class PhysicsComponent : EntityComponent, IPhysicsComponent
    {
        ...
        public PhysicsComponent(IEntity Parent) : base(Parent)
        {
            ...
            _gravity = Gravity;
            using (var bodyInfo = new RigidBodyConstructionInfo(1, _motionState = new PhysicsComponentMotionState(),
                new BoxShape(Vector3.One.Compatible())))
            {
            ...
            using (var bodyInfo =
                new RigidBodyConstructionInfo(1, new DefaultMotionState(), new BoxShape(Vector3.One.Compatible())))
            {
        ...
        {
            get => (_mainInformation.Mask & CollisionFilterGroups.CharacterFilter) ==
        ...
        public override void Update()
        {
            ...
            _speedMultiplier = Mathf.Lerp(_speedMultiplier,
                NormalSpeedModifier * (Parent.IsAttacking ? Parent.AttackingSpeedModifier : 1), deltaTime * 2f);
        ...
        public bool CollidesWithOffset(Vector3 Offset)
        {
            bool DoRaycast(Vector3 From)
            {
                ...
                return _rayResult.CollisionObjects.Count(
                    C => !ReferenceEquals(C, _sensor) && !ReferenceEquals(C, _body)) > 0;
            }
        ...
        public bool StaticRaycast(Vector3 End, out Vector3 Hit)
        {
            Hit = End;
            var result =
                BulletPhysics.Raycast(
                    RigidbodyPosition.Compatible() + Vector3.UnitY.Compatible() * Parent.Model.Height * .5f,
                    End.Compatible(), CollisionFilterGroups.StaticFilter | BulletPhysics.TerrainFilter);
            if (result.HasHit)
        ...
        private void OnCollision(CollisionObject Object0, CollisionObject Object1)
        {
            ...
            if (!ReferenceEquals(other, _body)) _sensorContacts++;
        }
        ...
        private void HandleIsStuck()
        {
            ...
            {
                IsStuck = _moved && _body.LinearVelocity.Compatible().Xz().LengthSquared() < 4f && !Parent.IsKnocked;
                _moved = false;
            }
        }
        ...
        {
            if (_body.LinearVelocity.Compatible().Xz().LengthSquared() > 1f) OnMove?.Invoke();
        }
            ...
            else if (FallTime > 0)
            {
                if (FallTime > 2.0f && HasFallDamage)
                {
                    if (!Parent.SearchComponent<DamageComponent>()?.Immune ?? true)
                    {
                        ...
                        {
                            Parent.Damage(fallTime * 12.5f, null, out _);
