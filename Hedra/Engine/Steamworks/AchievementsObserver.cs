...
namespace Hedra.Engine.Steamworks
{
    ...
    {
        private static readonly HashSet<string> CompletedAchievements = new HashSet<string>();
        ...
        private static void OnPlayerKill(DeadEventArgs Args)
        {
            ...
            else if (Args.DamageType == DamageType.Bleed) TriggerAchievementIfNecessary(Achievement.ACH_DEEP_CUT);
            AddStat(Statistic.KILL_COUNT, 1);
        }

        private static void OnPlayerOnDeath(DeadEventArgs Args)
        {
            if (Args.DamageType == DamageType.FoodPoison) TriggerAchievementIfNecessary(Achievement.ACH_LESSON_LEARNED);
        ...
        private static void OnSpecializationLearned(AbilityTreeBlueprint Blueprint)
        {
            ...
            var hasAll = true;
            foreach (var pair in map) hasAll &= HasAchievement(pair.Value);
        ...
        private static void OnCompanionChanged(Item CompanionItem, IEntity Companion)
        {
            ...
            if (string.Equals(CompanionItem.Name, ItemType.CompanionBee.ToString(),
        ...
        {
            if (!Quest.HasNext) AddStat(Statistic.QUEST_COUNT, 1);
        }
        ...
        {
            if (Type == TransactionType.Buy) AddStat(Statistic.GOLD_SPENT, Price);
        }
