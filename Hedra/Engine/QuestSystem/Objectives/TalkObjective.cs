    ...
    public class TalkObjective : Objective
    {
        ...
        public override uint QuestLogIcon
        {
            get
            {
                ...
                SceneManager.Game.LPlayer.UI.DrawPreview(_talkTarget.Model, UserInterface.QuestFbo);
        ...
        public override bool ShouldDisplay =>
            (SceneManager.Game.LPlayer.Position - IconPosition).LengthSquared > 96 * 96;
        ...
        private IEnumerator Update()
        {
            while (!_talked)
            {
                ...
                if (!_isFollowing && (SceneManager.Game.LPlayer.Position - IconPosition).LengthSquared <
                    96 * 96)
                {
                    _talkTarget.AddComponent(new FollowAIComponent(_talkTarget, SceneManager.Game.LPlayer));               
                    _isFollowing = true;
                }

                _talkTarget.Model.Tint = new Vector4(1.0f, 1.0f, 1.0f, 1.0f);

                if ((_talkTarget.Position - SceneManager.Game.LPlayer.Position).LengthSquared < 10 * 10
                    && Vector3.Dot((_talkTarget.Position - SceneManager.Game.LPlayer.Position).NormalizedFast(),
                        SceneManager.Game.LPlayer.View.LookAtPoint.NormalizedFast()) > .6f)
                {
