...
namespace Hedra.Engine.QuestSystem.Objectives
{
	...
	public class BossObjective : Objective
	{
		...
		public override bool ShouldDisplay{
			get{
				if(SceneManager.Game.LPlayer == null) return false;
				
				return !(Boss != null && Boss.IsDead) || !(Boss != null && (SceneManager.Game.LPlayer.Position - Boss.Position).LengthSquared < 24*24);
			}
		}
		...
		public override uint QuestLogIcon {
			get {
				if(_bossType == MobType.Bee || _bossType == MobType.Rat){
				    ...
				    SceneManager.Game.LPlayer.UI.DrawPreview(Boss.Model, UserInterface.QuestFbo);
				    model.Model.Scale /= 2;
				}else{
				    var model = Boss.Model as QuadrupedModel;
				    if (model == null) return UserInterface.QuestFbo.TextureID[0];
