...
namespace Hedra.Engine.QuestSystem.Objectives
{
    ...
    public class VillageObjective : Objective
    {
        ...
        public override uint QuestLogIcon
        {
            get
            {
                SceneManager.Game.LPlayer.UI.DrawPreview(_iconMesh, UserInterface.QuestFbo);
        ...
        private IEnumerator Update()
        {
            while (!Disposed)
            {
                ...
                if (Escortee == null && (SceneManager.Game.LPlayer.Position - Position).LengthSquared < 64 * 64)
                {
                    this.NextObjective();
                    break;
                }
                if (!_welcomeMsgShown && (SceneManager.Game.LPlayer.Position - Position).LengthSquared < 192 * 192)
                {
