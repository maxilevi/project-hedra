    ...
    public class NormalBiomeGenerationDesign : BaseBiomeGenerationDesign
    {
            ...
            int Width, int Height, float HorizontalScale, float VerticalScale, Vector3 Offset)
        {
            ...
            set = MultiplySetsDimensional(
                set,
                TransformSet(Noise.GetSimplexSetWithFrequency(Offset.Xz(), size.Xz(), scale.Xz(), 0.0008f),
                    F => F.Clamp01() * 3f),
                size
            );
            ...
            int Width, float Scale, Vector2 Offset)
        {
            ...
            var bigMountain =
                Noise.GetPerlinSetWithFrequency(Offset, new Vector2(Width, Width), new Vector2(Scale, Scale), 0.0005f);
