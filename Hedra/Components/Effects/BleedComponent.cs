...
namespace Hedra.Components.Effects
{
    ...
    {
        public BleedComponent(IEntity Entity, int Chance, float Damage, float Duration) : base(Entity, Chance, Damage,
            Duration)
        {
        }
        ...
        protected override void DoApply(IEntity Victim, float Amount)
        {
            if (Victim.SearchComponent<BleedingComponent>() == null)
                Victim.AddComponent(new BleedingComponent(Victim, Parent, Duration + Utils.Rng.NextFloat() * 4 - 2f,
                    Damage));
        }
    }
}