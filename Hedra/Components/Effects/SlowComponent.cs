...
namespace Hedra.Components.Effects
{
    ...
    {
        public SlowComponent(IEntity Entity, int Chance, float Damage, float Duration) : base(Entity, Chance, Damage,
            Duration)
        {
        }
        ...
        protected override void DoApply(IEntity Victim, float Amount)
        {
            if (Victim.SearchComponent<SlowingComponent>() == null)
                Victim.AddComponent(new SlowingComponent(Victim, Parent, Duration + Utils.Rng.NextFloat() * 4 - 2f,
                    Damage));
        }
    }
}