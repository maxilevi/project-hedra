...
namespace Hedra.Components.Effects
{
    public class SpeedComponent : ApplyEffectComponent
    {
        ...
        public SpeedComponent(IEntity Entity, int Chance = 10, float SpeedBonus = .5f, float BaseSpeedTime = 4) : base(Entity, Chance, SpeedBonus, BaseSpeedTime)
        {
        }
        ...
        protected override void DoApply(IEntity Victim, float Amount)
        {
            ...
            if(Parent.SearchComponent<SpeedBonusComponent>() == null)
                Parent.ComponentManager.AddComponentWhile(new SpeedBonusComponent(Parent, Damage), () => _speedTime > 0);
        }
    }
}
